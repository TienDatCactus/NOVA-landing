...
export default function Faqs1() {
  ...
  return (
    <div
      className="p-5 mx-auto rounded-2xl max-w-xl shadow-sm mt-4"
      ref={faqsRef}
    >
      <article className="max-w-2xl mx-auto text-center space-y-3 pb-10 pt-4">
        <TimelineContent
          as="span"
          animationNum={0}
          timelineRef={faqsRef}
        ...
        <Accordion defaultValue="item-2">
          {faqData.map((item, index) => (
            <AccordionItem
              key={index}
              value={`item-${index}`}
              className="mb-0 rounded-xl bg-neutral-100 border border-neutral-200 w-full data-[active]:pb-3 mt-3"
            >
              <TimelineContent
                as="div"
              ...
              >
                <AccordionHeader
                  customIcon
                  className="hover:no-underline p-0 pl-4 py-1 relative data-[active]:bg-transparent hover:bg-transparent text-neutral-950 hover:text-neutral-950  data-[active]:text-neutral-950 sm:text-base text-sm"
                >
